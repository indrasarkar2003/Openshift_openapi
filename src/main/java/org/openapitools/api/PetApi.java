...
public interface PetApi {
    ...
    default Mono<ResponseEntity<Pet>> getPetById(@ApiParam(value = "ID of pet to return",required=true) @PathVariable("petId") Long petId) {
    	ResponseEntity<Pet> responseEntitySub=null;
    	Optional<NativeWebRequest> optRequest=getRequest();
    	if(optRequest.isPresent()) {
    		NativeWebRequest request=optRequest.get();
    		for (MediaType mediaType: MediaType.parseMediaTypes(request.getHeader("Accept"))) {
                if (mediaType.isCompatibleWith(MediaType.valueOf("application/json"))) {
            		...
            		HttpEntity<?> requestEntitySub = new HttpEntity<Object>(requestHeaders);
            		
            		responseEntitySub = restTemplate.exchange(URI.create(Constant.PETSTORE_URI+"pet/" + petId.toString()), HttpMethod.GET, requestEntitySub, Pet.class);
            		
                    break;
                }
            }
    	}
    	Mono<ResponseEntity<Pet>> asyResponse = Mono.just(responseEntitySub);
        return asyResponse;
    }
